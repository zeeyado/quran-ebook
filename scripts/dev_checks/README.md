...
| `check_plugin_helpers.lua` | Plugin v1.11 helpers (`markerPuaCodepoint`, `normalizeArabicName`) — extracted LIVE from main.lua so tested code can't drift. Run: `/Applications/KOReader.app/Contents/koreader/luajit scripts/dev_checks/check_plugin_helpers.lua`. Path roots auto-detected (works from a split plugin repo too; override `QURAN_PLUGIN_DIR`/`QURAN_EBOOK_DIR`/`KOREADER_DIR`) — slated to move to the quran.koplugin repo as `dev/` at Phase S | `tools/quran.koplugin/main.lua` |
